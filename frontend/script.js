...
async function add_to_cart(data) {
    const response = await fetch("http://127.0.0.1:8000/Products/1/add_to_cart?username=user&quantity=1");
    console.log(data)
    const jsonData = await response.json();
    create_cart_card(jsonData)
}

function create_cart_card(data) {
    itemHTML = ""
    data.map((item) =>
        itemHTML += `<div class="item">
        <img src="https://image.makewebeasy.net/makeweb/r_409x409/pyuHEt3NP/SEO/SEO.jpg?v=202012190947" width="300"/>
        ...
        <p>${item.detail}</p>
        <p>${item.price}</p>
        <button class="buy" onclick=${add_to_cart(item)}>
            <img
                src="../frontend/cart-shopping-solid.svg"
                width="10"
                alt="cart"
